...
public class CommandMauwarsAdmin implements IngameCommandExecutor {
	...
	public boolean onCommand(Player p, Command cmd, String label, String[] args) {
		if (args.length > 0) {
			if (args[0].equalsIgnoreCase("pos1")) {
				...
				p.sendMessage(plugin.stag + plugin.translate("creating.pos", 1));
				return true;
			} else if (args[0].equalsIgnoreCase("pos2")) {
				p.setMetadata(plugin.pos2_meta, new FixedMetadataValue(plugin, p.getLocation().getBlock().getLocation()));
				p.sendMessage(plugin.stag + plugin.translate("creating.pos", 2));
				return true;
				...
				if (args[0].equalsIgnoreCase("create")) {
					MetadataValue c1 = MetadataUtils.getMetadata(p, plugin.pos1_meta, plugin);
					MetadataValue c2 = MetadataUtils.getMetadata(p, plugin.pos2_meta, plugin);
					if (c1 == null || c2 == null || c1.value() == null || c2.value() == null || !(c1.value() instanceof Location)
							|| !(c2.value() instanceof Location)) {
						p.sendMessage(plugin.errtag + plugin.translate("creating.selection", label));
						return true;
					}
					...
					if (ma != null) {
						...
						p.sendMessage(plugin.stag + plugin.translate("spawn.added", ma.getName()));
					} else p.sendMessage(plugin.errtag + plugin.translate("arena.notfound", args[1]));
					return true;
				} else if (args[0].equalsIgnoreCase("remove")) {
					if (plugin.removeArena(args[1])) p.sendMessage(plugin.stag + plugin.translate("arena.removed", args[1]));
					else p.sendMessage(plugin.errtag + plugin.translate("arena.notfound", args[1]));
					return true;
				} else if (args[0].equalsIgnoreCase("disable")) {
					MauArena ma = plugin.getArena(args[1]);
					if (ma != null) {
						if (ma.disable()) p.sendMessage(plugin.stag + plugin.translate("disable.success"));
						else p.sendMessage(plugin.stag + plugin.translate("disable.fail"));
					}
					...
					} else p.sendMessage(plugin.errtag + plugin.translate("arena.notfound"));
					...
					try {
						...
						p.sendMessage(plugin.errtag + plugin.translate("cc.nfe"));
						return true;
					};
					p.sendMessage(plugin.stag + plugin.translate("cc.added", plugin.getChestContentHandler().addEntry(p.getItemInHand(), d)));
